...
import { triggerTwitterStatusCheck } from './actions/twitter';
...
function TwitterSettings({ onError, onSuccess }: { onError: (msg: string) => void, onSuccess: (success: boolean) => void }) {
  ...
  async function handleCheckStatus() {
    ...
    const res = await triggerTwitterStatusCheck();
    if (res.success) {
      loadAccounts();
      onSuccess(true);
      setTimeout(() => onSuccess(false), 2000);
    } else {
      onError(res.error || 'Fehler beim Prüfen');
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between mb-3">
        <h3 className="font-medium text-[var(--text-muted)] text-sm uppercase tracking-wider">Verknüpfte Accounts</h3>
        <button
          onClick={handleCheckStatus}
          disabled={checkingStatus}
          className="text-xs flex items-center gap-1.5 px-3 py-1.5 bg-blue-500/10 text-blue-400 hover:bg-blue-500/20 rounded-lg transition-all border border-blue-500/20 disabled:opacity-50"
        >
          {checkingStatus ? <div className="spinner w-3 h-3 border-blue-400" /> : (
            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
          )}
          {checkingStatus ? 'Prüfe...' : 'Sitzung prüfen'}
        </button>
      </div>
      {accounts.length === 0 ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    {acc.lastLoginStatus !== undefined && (
                      <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold uppercase tracking-wider ${acc.lastLoginStatus ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'
                        }`}>
                        {acc.lastLoginStatus ? 'Eingeloggt' : 'Login Nötig'}
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-[var(--text-muted)] flex items-center gap-2">
                    {acc.displayName}
                    {acc.lastStatusCheckAt && (
                      <span className="text-[10px] opacity-50">
                        (Stand: {new Date(acc.lastStatusCheckAt).toLocaleString('de-DE', { hour: '2-digit', minute: '2-digit' })})
                      </span>
                    )}
                  </div>
                </div>
...
interface MonitoringLog {
  ...
  followingCountLive: number | null;
  followingCountDb: number | null;
  scrapedCount: number | null;
  scrapeQuote: number | null;
  newFollowsCount: number;
  unfollowsCount: number;
  newFollows: string[] | null;
  unfollows: string[] | null;
  errorMessage: string | null;
  durationMs: number | null;
  createdAt: string;
}

interface MonitoringLogsPanelProps {
  profileId: string;
  isOpen: boolean;
  onClose: () => void;
}

function MonitoringLogsPanel({ profileId, isOpen, onClose }: MonitoringLogsPanelProps) {
  ...
  const loadLogs = async () => {
    ...
    try {
      ...
      const url = `/api/monitoring-logs?profileId=${profileId}${fromDate ? `&from=${fromDate}` : ''}`;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'NO_CHANGE': return 'text-gray-400';
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div
        className="glass-card p-6 w-full max-w-2xl mx-4 max-h-[80vh] overflow-hidden flex flex-col"
        onClick={e => e.stopPropagation()}
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-bold">📊 Monitoring Logs</h2>
          <button onClick={onClose} className="p-2 hover:bg-[var(--card)] rounded-lg">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
        ...
        <div className="flex gap-2 mb-4">
          {(['7d', '30d', 'all'] as const).map(f => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${dateFilter === f
                ? 'bg-[var(--accent)] text-white'
                : 'bg-[var(--card)] text-[var(--text-muted)] hover:text-white'
                }`}
            >
              {f === '7d' ? 'Letzte 7 Tage' : f === '30d' ? 'Letzte 30 Tage' : 'Alle'}
            </button>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-2">
          {loading ? (
            ...
            logs.map(log => (
              <div key={log.id} className="p-3 bg-[var(--card)] rounded-xl border border-[var(--border)]">
                ...
                <div className="grid grid-cols-3 gap-2 text-sm">
                  {log.scrapeQuote !== null && (
                    <div className="bg-[var(--background)] p-2 rounded-lg">
                      <span className="text-[var(--text-muted)]">Quote: </span>
                      <span className={log.scrapeQuote >= 95 ? 'text-green-400' : 'text-yellow-400'}>
                        {log.scrapeQuote.toFixed(1)}%
                      </span>
                    </div>
                  )}
                  {log.newFollowsCount > 0 && (
                    <div className="bg-[var(--background)] p-2 rounded-lg">
                      <span className="text-green-400">+{log.newFollowsCount} Neu</span>
                    </div>
                  )}
                  {log.unfollowsCount > 0 && (
                    <div className="bg-[var(--background)] p-2 rounded-lg">
                      <span className="text-red-400">-{log.unfollowsCount} Entfolgt</span>
                    </div>
                  )}
                </div>
...
function SetDetail({ set, onBack, onRefresh, onShowDetails }: SetDetailProps) {
  ...
  return (
    <>
      ...
      {/* Monitoring Logs Panel */}
      <MonitoringLogsPanel
        profileId={logsProfileId || ''}
        isOpen={!!logsProfileId}
        onClose={() => setLogsProfileId(null)}
      />
    </>
  );
}
...
function ProfileDetailsModal({ isOpen, onClose, onRefresh, profileId, username }: ProfileDetailsModalProps) {
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div
        className={`glass-card w-full ${isStatsTab ? 'max-w-5xl lg:max-w-6xl h-[90vh] md:h-[92vh]' : 'max-w-2xl h-[85vh]'} flex flex-col mx-4 animate-scale-in overflow-hidden`}
        onClick={e => e.stopPropagation()}
      >
        {/* Header Section */}
        ...
        <div className={`flex-1 min-h-0 ${isStatsTab ? 'overflow-hidden' : 'overflow-y-auto'} p-8 custom-scrollbar bg-[var(--background)]`}>
          {loading ? (
              ...
              {activeTab === 'stats' && (
                <div className="h-full min-h-0 flex flex-col gap-4 animate-in fade-in duration-500 overflow-y-auto pb-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div className="bg-[var(--card)] p-4 rounded-2xl border border-[var(--border)] shadow-sm hover:border-[var(--success)]/30 transition-all">
                    ...
                    <div className="bg-[var(--card)] p-4 rounded-2xl border border-[var(--border)] shadow-sm hover:border-[var(--error)]/30 transition-all">
                    ...
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-4">
                      <div>
                        <h3 className="text-base font-bold flex items-center gap-2">
                          <svg className="w-5 h-5 text-[var(--accent-solid)]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            title="CSV Bericht"
                            aria-label="CSV Bericht"
                            className="p-2 bg-[var(--background)] border border-[var(--border)] rounded-lg text-[var(--text-muted)] hover:text-[var(--foreground)] hover:border-[var(--accent-solid)]/40 transition-all"
                          >
                          ...
                          <button
                            ...
                            className="p-2 bg-[var(--foreground)] text-[var(--background)] rounded-lg hover:opacity-90 transition-all"
                          >
                            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 16h8m-8-4h8m2 8H6a2 2 0 01-2-2V6a2 2 0 012-2h8l6 6v8a2 2 0 01-2 2z" />
                            </svg>
                          </button>
                        </div>
                      </div>
                    ...
                    <div className="w-full" style={{ minHeight: '350px' }}>
                      {historyList.length < 1 ? (
                        ...
                        for (let i = 0; i < days; i++) {
                          ...
                          const followingAtTime = currentCount
                            - changesAfter.filter(h => h.type === 'FOLLOW').length
                            + changesAfter.filter(h => h.type === 'UNFOLLOW').length;
